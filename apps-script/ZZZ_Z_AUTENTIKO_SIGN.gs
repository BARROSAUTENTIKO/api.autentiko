...
function AUT_SIGN_LIVENESS_VALIDO_(liveness) {
  ...
  var metodoFace = String(liveness.metodoFace || (liveness.rosto && liveness.rosto.metodo) || '');
  var detectorFacialForte = metodoFace === 'MediaPipeFaceDetector' || metodoFace === 'FaceDetector';
...
function apiSignCriarSessao(idLaudo, signatarios, posicoes, usuario) {
  try {
    ...
    if (typeof AUT_STATUS_LAUDO_ATUAL_V7_ === 'function') {
      var statusLaudo = AUT_STATUS_LAUDO_ATUAL_V7_(ss, idReal, numero, rowObj, AUT_RECUPERAR_PAYLOAD_DE_ACHADO_(achado));
      if (statusLaudo === 'RASCUNHO') {
        return { sucesso: false, msg: 'Este laudo ainda está em rascunho. Finalize o laudo antes de criar assinatura eletrônica.' };
      }
    }
...
function AUT_SIGN_ANALISAR_RISCO_(contexto, passkey, liveness, consentimentos) {
  ...
  var metodoFace = String(liveness.metodoFace || (liveness.rosto && liveness.rosto.metodo) || '');
  if (metodoFace && metodoFace.indexOf('heuristica') >= 0) flags.push('ROSTO_POR_HEURISTICA_LOCAL');
  if (metodoFace && metodoFace !== 'MediaPipeFaceDetector' && metodoFace !== 'FaceDetector') flags.push('DETECTOR_FACIAL_FRACO_OU_INDISPONIVEL');
